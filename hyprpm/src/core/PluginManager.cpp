...
bool CPluginManager::updateHeaders(bool force) {
    ...
    progress.printMessageAbove(statusString("!", Colors::YELLOW, "Cloning https://github.com/littleblack111/Hyprland, this might take a moment."));
    ...
    std::string ret = execAndGet(std::format("cd {} && git clone --recursive https://github.com/littleblack111/Hyprland hyprland-{}{}", getTempRoot(), USERNAME,
                                             (bShallow ? " --shallow-since='" + SHALLOW_DATE + "'" : "")));

    if (!std::filesystem::exists(WORKINGDIR)) {
        progress.printMessageAbove(failureString("Clone failed. Retrying without shallow."));
        ret = execAndGet(std::format("cd {} && git clone --recursive https://github.com/littleblack111/hyprland hyprland-{}", getTempRoot(), USERNAME));
    }
